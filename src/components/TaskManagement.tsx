...
import { getCTASTasks, createCTASTask, updateCTASTaskStatus } from '../utils/database';
...
const TaskManagement: React.FC = () => {
  ...
  const fetchTasks = async () => {
    const fetchedTasks = getCTASTasks();
  ...
  const handleCreateTask = (e: React.FormEvent) => {
    ...
    if (newTask.title && newTask.description) {
      const createdTask = createCTASTask(
  ...
  const handleUpdateTaskStatus = (taskId: string, newStatus: 'Pending' | 'In Progress' | 'Completed') => {
    const updatedTask = updateCTASTaskStatus(taskId, newStatus);
    if (updatedTask) {
