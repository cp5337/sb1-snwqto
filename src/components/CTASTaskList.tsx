...
import { getCTASTasks, createCTASTask, updateCTASTaskStatus, CTASTask } from '../utils/database';
...
const CTASTaskList: React.FC = () => {
  ...
  const handleAddTask = async (e: React.FormEvent) => {
    ...
    if (newTaskTitle && newTaskDescription) {
      try {
        await createCTASTask((tasks.length + 1).toString(), newTaskTitle, newTaskDescription, false);
        setNewTaskTitle('');
  ...
  const handleUpdateStatus = async (taskId: string, newStatus: 'Pending' | 'In Progress' | 'Completed') => {
    try {
      await updateCTASTaskStatus(taskId, newStatus);
      fetchTasks();
    } catch (err) {
