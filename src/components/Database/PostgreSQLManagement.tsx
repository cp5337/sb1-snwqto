...
import { Plus, Trash2, Play, Square, Table } from 'lucide-react';

interface PostgreSQLInstance {
  id: string;
  name: string;
...
const PostgreSQLManagement: React.FC = () => {
  const [instances, setInstances] = useState<PostgreSQLInstance[]>([
    { id: '1', name: 'UserManagement', status: 'Active', version: '14.3', port: 5432, databases: 3, tables: 50 },
    { id: '2', name: 'AuditLogs', status: 'Active', version: '14.3', port: 5433, databases: 1, tables: 20 },
    { id: '3', name: 'ConfigManagement', status: 'Inactive', version: '13.7', port: 5434, databases: 2, tables: 30 },
  ]);

  const [newInstance, setNewInstance] = useState<Omit<PostgreSQLInstance, 'id'>>({
    name: '',
    status: 'Inactive',
    version: '',
    port: 5432,
  ...
  return (
    <div className="space-y-4">
        ...
        {instances.map(instance => (
          <div key={instance.id} className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-md text-xs">
            ...
            <div className="space-y-1">
              ...
              <p><span className="font-semibold">Databases:</span> {instance.databases}</p>
              <p><span className="font-semibold">Tables:</span> {instance.tables}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-md">
        <h2 className="font-semibold mb-2">Add New PostgreSQL Instance</h2>
...
export default PostgreSQLManagement;