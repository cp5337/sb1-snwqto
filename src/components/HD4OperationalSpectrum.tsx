    ...
    <div className="bg-gray-800 p-8 rounded-lg shadow-lg space-y-8">
      ...
      <div className="relative h-[600px]">
        <div className="absolute top-0 left-0">
          <Stage {...stages[0]} onNoiseChange={(noise) => handleNoiseChange(0, noise)} />
        </div>
        <div className="absolute top-0 right-0">
          <Stage {...stages[1]} onNoiseChange={(noise) => handleNoiseChange(1, noise)} />
        </div>
        <div className="absolute bottom-0 left-0">
          <Stage {...stages[2]} onNoiseChange={(noise) => handleNoiseChange(2, noise)} />
        </div>
        <div className="absolute bottom-0 right-0">
          <Stage {...stages[3]} onNoiseChange={(noise) => handleNoiseChange(3, noise)} />
        </div>
        <div className="absolute bottom-1/2 left-1/2 transform -translate-x-1/2 translate-y-1/2">
          <TargetRings stages={stages} />
        </div>
        <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2">
          <Stage {...stages[4]} onNoiseChange={(noise) => handleNoiseChange(4, noise)} />
        </div>
      </div>
