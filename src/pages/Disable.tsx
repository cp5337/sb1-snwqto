      ...
      {activeTab === 'overview' && (
        <>
          <div className="h-3/4 mb-4">
            {view === 'map' && <HD4Map hd4Action="Disable" />}
            {view === 'graph' && <CTASGraphAnimation />}
          </div>
          <div className="h-1/4">
            <HD4TaskView hd4Action="Disable" />
          </div>
        </>
      )}

      {activeTab === 'scripts' && <ScriptScrapers hd4Action="Disable" />}

      {activeTab === 'transforms' && <TransformsGraph hd4Action="Disable" />}

      {activeTab === 'tools' && <KaliToolsIntegration hd4Action="Disable" />}
    </div>
  );
};
