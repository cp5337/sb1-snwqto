      ...
      {activeTab === 'overview' && (
        <>
          <div className="h-3/4 mb-4">
            {view === 'map' && <HD4Map hd4Action="Detect" />}
            {view === 'graph' && <CTASGraphAnimation />}
          </div>
          <div className="h-1/4">
            <HD4TaskView hd4Action="Detect" />
          </div>
        </>
      )}

      {activeTab === 'scripts' && <ScriptScrapers hd4Action="Detect" />}

      {activeTab === 'transforms' && <TransformsGraph hd4Action="Detect" />}

      {activeTab === 'tools' && <KaliToolsIntegration hd4Action="Detect" />}
    </div>
  );
};
