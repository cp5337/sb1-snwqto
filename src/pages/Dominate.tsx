      ...
      {activeTab === 'overview' && (
        <>
          <div className="h-3/4 mb-4">
            {view === 'map' && <HD4Map hd4Action="Dominate" />}
            {view === 'graph' && <CTASGraphAnimation />}
          </div>
          <div className="h-1/4">
            <HD4TaskView hd4Action="Dominate" />
          </div>
        </>
      )}

      {activeTab === 'scripts' && <ScriptScrapers hd4Action="Dominate" />}

      {activeTab === 'transforms' && <TransformsGraph hd4Action="Dominate" />}

      {activeTab === 'tools' && <KaliToolsIntegration hd4Action="Dominate" />}
    </div>
  );
};
